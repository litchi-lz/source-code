    ...
    <aside>
      ...
      <div class="menu-list">
        ...
        <div @click="intoDiaryPage()" class="menu-item">日记</div>
         <router-link to="/login" class="menu-item">Go to About</router-link>
      </div>
      ...
      <div :style="{ 'transform': 'translateY(' + blogLayoutY + ')' }" class="blog-layout layout-item">
        <nav>
          <div class="list" :style="{
            ...
            <li @click="steadyType(1, item.type, index)" v-for="(item, index) in steadyList" :key="item"
              :class="{ 'select': currentNavIndex == index }">
              {{ item.mold }}
            </li>
        ...
        <div class="container">
          ...
          <el-main class="el-main" v-loading="loading">
            <div :class="{ 'hide-article-list': hideAarticleList }" class="article-list">
              <div v-for="item in blogs" :key="item" @click="onArticleItemClick(item.mdId, item.blogTitle)"
                class="article-item">
                <header>
              ...
              <button :class="{ select: currentPage == index }"
                @click="listByType(index, currentClassify, currentNavIndex)" class="page-button"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onArticleItemClick = (id, articleTitle) => {
      getBlogContent(id, articleTitle);
    };
    /**
     * 获取博客内容
     */
    const getBlogContent = (id, articleTitle) => {
      state.loading = true;
      state.loading = false;
      state.parameter = id;
      state.currentBlogTitle = articleTitle;
      state.showTitle = true;
      state.hideNavBar = true;
    ...
    const listByType = (page, type, navIndex) => {
      console.log(page, type, navIndex)
    ...
    const steadyType = (page, type, navIndex) => {
      state.loading = true
      state.currentNavIndex = navIndex;
      state.blogs = muster[type];
